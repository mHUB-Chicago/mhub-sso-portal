...
export const SelectCompanyStep = ({ value, onChange }: SelectCompanyStepProps) => {
  const { data, isLoading, error } = useGetCompaniesQuery({ limit: 500, active: "true" });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="mb-2 text-2xl font-semibold">Select Company</h2>
        <p className="text-gray-600">Pick the existing company this member belongs to. No company details needed.</p>
      </div>
      ...
      <div>
        ...
        {isLoading ? (
          ...
          <p className="mt-1.5 text-xs text-red-500">Failed to load companies.</p>
        ) : (
