...
export const handleGetCompanies = async (c: Context<AppType, string, QueryInput<typeof GetCompaniesRequestSchema>>) => {
  const { limit, offset, search, membershipType, active, noEmail, cmtOnly, subscriptionStatus } = c.req.valid("query");
  const result: GetPaginatedCompaniesResult = await getPaginatedCompanies(c, { limit, offset, search, membershipType, active, noEmail, cmtOnly, subscriptionStatus });
