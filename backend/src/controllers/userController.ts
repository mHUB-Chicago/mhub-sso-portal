...
export const handleGetUsers = async (c: Context<AppType, string, QueryInput<typeof GetUsersRequestSchema>>) => {
  const { limit, offset, role, search, companyId, primaryMembership, active, emailVerified, portalAccess, noEmail, noName, noPrimary, directPersonal, unresolved, memberSource, cmtOnly, primaryMembershipStatus } = c.req.valid("query");
  const result: GetPaginatedUsersResult = await getPaginatedUsers(c, { limit, offset, role, search, companyId, primaryMembership, active, emailVerified, portalAccess, noEmail, noName, noPrimary, directPersonal, unresolved, memberSource, cmtOnly, primaryMembershipStatus });
